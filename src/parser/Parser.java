...
public class Parser {
   ...
   void match(int t) throws IOException {
      if( look.tag == t ) move(); //tag
      else error("syntax error");
   }

   public void program() throws IOException {  // program -> block  ����program ��block
      Stmt s = block();
      int begin = s.newlabel();  int after = s.newlabel();
      s.emitlabel(begin);  s.gen(begin, after);  s.emitlabel(after); //�м��������
   }

   Stmt block() throws IOException {  // block -> { decls stmts }
      match('{');  Env savedEnv = top;  top = new Env(top); //top��������ű� savedEnvָ��ǰ����ű������
      decls(); Stmt s = stmts();
      match('}');  top = savedEnv;
      return s;
   }
